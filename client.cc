...
int32_t prepare_set_cmd(const char *key, const char *val, char *wbuf)
{
  ...
  printf("REQUEST : Command 'set' with key '%s' and value '%s'\n", key, val);
...
int32_t prepare_get_cmd(const char *key, char *wbuf)
{
  ...
  assert(offset == total_message_len);
  printf("REQUEST : Command 'get' with key '%s'\n", key);
  return total_message_len;
}
...
int32_t prepare_del_cmd(const char *key, char *wbuf)
{
  ...
  size_t total_message_len = 4 + (4 + 3) + (4 + key_len);
  assert(offset == total_message_len);
  printf("REQUEST : Command 'del' with key '%s'\n", key);
  return total_message_len;
}
...
int32_t prepare_malformed(char *wbuf)
{
  ...
  printf("REQUEST : Malformed command '%s'\n", CMD);
  return offset;
}
...
int32_t receive_reply(int fd)
{
  ...
  rbuf[len] = '\0';
  printf("REPLY   : Server responded with message '%s'\n", rbuf);
  return 0;
}
...
int runTests(int fd)
{
  ...
  if (send_command(fd, buf, message_len) != 0)
    return -1;
  ...
  if (send_command_fragmented(fd, buf, message_len) != 0)
    return -1;
  if (receive_reply(fd) != 0)
    return -1;
